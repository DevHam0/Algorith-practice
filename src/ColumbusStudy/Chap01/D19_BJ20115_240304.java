...
public class D19_BJ20115_240304 {
    /*
...
//절대/상대 오차는 10-5까지 허용한다.
    public static void main(String[] args) throws IOException {
        ...
        double max = getMax(quantities);
        double answer = getAnswer(quantities, max);
    ...
    static double getMax(String[] quantities) {
        double max = 0;
    ...
    static double getAnswer(String[] quantities, double max) {
        double sum = 0;
        for(String quantity : quantities) {
            if (Double.parseDouble(quantity) == max) {
                sum += max;
            } else {
                sum += Double.parseDouble(quantity)/2;
            }
        }
        return sum;
    }
