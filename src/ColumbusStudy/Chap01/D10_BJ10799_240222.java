...
두 개의 쇠막대기는 각각 3개와 2개의 조각으로 잘려지고, 이와 같은 방식으로
주어진 쇠막대기들은 총 17개의 조각으로 잘려진다.
...
조각의 총 개수를 구하는 프로그램을 작성하시오.
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i=0; i<input.length(); i++) {
            ...
            if (c == '(') { // 열린 괄호면 open에 1을 더해줌
                open++;
            } else { // 닫힌 괄호일 경우
                open--; // 일단 open에 1을 빼줌

                if (input.charAt(i - 1) == '(') { // 그 전 괄호가 열린 괄호면 레이저를 의미
                    ans += open; // open의 값만큼 더해줌
                } else {
                    ans++; // 단순히 1을 더해줌
                }
            }
